...
MIGRATION_MARKER = (
    "# managed by et-agent — `hermes codex-runtime migrate` regenerates this section"
)
MIGRATION_END_MARKER = (
    "# end et-agent managed section"
)
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (ET_AGENT_HOME, PYTHONPATH) could
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a ET_AGENT_HOME pointing
...
def _build_hermes_tools_mcp_entry() -> dict:
    ...
    ET_AGENT_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # ET_AGENT_HOME passes through IF SET so the MCP subprocess sees the same
    ...
    # codex's subprocess to inherit whatever ET_AGENT_HOME its launcher sets
    ...
    # — that would override the launcher's ET_AGENT_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("ET_AGENT_HOME", tmp_path) would
    ...
    hermes_home = os.environ.get("ET_AGENT_HOME") or ""
    if hermes_home and _looks_like_test_tempdir(hermes_home):
        hermes_home = ""
    if hermes_home:
        env["ET_AGENT_HOME"] = hermes_home
...
def migrate(
    ...
    *,
    ...
    Args:
        hermes_config: full ~/.et-agent/config.yaml dict
