# ET-Agent - Development Guide

Instructions for AI coding assistants and developers working on the et-agent codebase.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.et-agent/plugins/` or a pip package, discovered at runtime.
...
`$HOME/.hermes/et-agent/venv` (for worktrees that share a venv with the
main checkout).
...
**User config:** `~/.et-agent/config.yaml` (settings), `~/.et-agent/.env` (API keys only).
**Logs:** `~/.et-agent/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
...
- Skill slash commands: `agent/skill_commands.py` scans `~/.et-agent/skills/`, injects as **user message** (not system prompt) to preserve prompt caching
...
Use the plugin route instead: create `~/.et-agent/plugins/<name>/plugin.yaml`
and `~/.et-agent/plugins/<name>/__init__.py`, then register tools with
...
~/.et-agent/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
Users create `~/.et-agent/skins/<name>.yaml`:
...
`~/.et-agent/plugins/` and pip-installed entry points.

### General plugins (`hermes_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.et-agent/plugins/`, `./.hermes/plugins/`,
...
into `~/.et-agent/plugins/` (or via pip entry points) — they implement
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "ET-Agent" is the secondary collaborator. If the
   contributor's commit shows "ET-Agent" as author (because they
...
lives in the `et-agent-dev` skill at
...
go to `~/.et-agent/skills/.archive/` and are restorable.
...
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.et-agent/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
- File lock at `~/.et-agent/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
2. **Use `display_hermes_home()` for user-facing messages.** Import from `hermes_constants`.
   This returns `~/.hermes` for default or `~/.et-agent/profiles/<name>` for profiles.
   ```python
   # GOOD
   ...
   print("Config saved to ~/.et-agent/config.yaml")
   ```
...
### Tests must not write to `~/.et-agent/`
The `_isolate_hermes_home` autouse fixture in `tests/conftest.py` redirects `HERMES_HOME` to a temp dir. Never hardcode `~/.et-agent/` paths in tests.
...
| HOME / `~/.et-agent/` | Your real config+auth.json | Temp dir per test |
