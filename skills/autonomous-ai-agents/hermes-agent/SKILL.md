---
name: et-agent
description: "Configure, extend, or contribute to ET-Agent."
version: 2.1.0
author: ET-Agent + Teknium
license: MIT
platforms: [linux, macos, windows]
metadata:
  hermes:
    tags: [hermes, setup, configuration, multi-agent, spawning, cli, gateway, development]
    homepage: https://github.com/NousResearch/et-agent
    related_skills: [claude-code, codex, opencode]
---

# ET-Agent

ET-Agent is an open-source AI agent framework by Nous Research that runs in your terminal, messaging platforms, and IDEs. It belongs to the same category as Claude Code (Anthropic), Codex (OpenAI), and OpenClaw — autonomous coding and task-execution agents that use tool calling to interact with your system. Hermes works with any LLM provider (OpenRouter, Anthropic, OpenAI, DeepSeek, local models, and 15+ others) and runs on Linux, macOS, and WSL.
...
**This skill helps you work with ET-Agent effectively** — setting it up, configuring features, spawning additional agent instances, troubleshooting issues, finding the right commands and settings, and understanding how the system works when you need to extend or contribute to it.

**Docs:** https://et-agent.nousresearch.com/docs/
...
(`hermes mcp install <name>`): `skill_view(name="et-agent", file_path="references/native-mcp.md")`.
...
Platform docs: https://et-agent.nousresearch.com/docs/user-guide/messaging/
...
patterns: `skill_view(name="et-agent", file_path="references/webhooks.md")`.
...
authoritative list or see the [live slash commands reference](https://et-agent.nousresearch.com/docs/reference/slash-commands).
...
/reload-skills       Re-scan ~/.et-agent/skills/ for added/removed skills
...
```
~/.et-agent/config.yaml       Main configuration
~/.et-agent/.env              API keys and secrets (under $HERMES_HOME if set)
$HERMES_HOME/skills/        Installed skills
~/.et-agent/sessions/         Gateway routing index, request dumps, *.jsonl transcripts (and optional per-session JSON snapshots when sessions.write_json_snapshots: true)
~/.et-agent/state.db          Canonical session store (SQLite + FTS5)
~/.et-agent/logs/             Gateway and error logs
~/.et-agent/auth.json         OAuth tokens and credential pools
~/.et-agent/et-agent/     Source code (if git-installed)
```

Profiles use `~/.et-agent/profiles/<name>/` with the same layout.
...
Full config reference: https://et-agent.nousresearch.com/docs/user-guide/configuration
...
Full provider docs: https://et-agent.nousresearch.com/docs/integrations/providers
...
Some shell-hook integrations require explicit allowlisting before they fire. Managed via `~/.et-agent/shell-hooks-allowlist.json` — prompted interactively the first time a hook wants to run.
...
User docs: https://et-agent.nousresearch.com/docs/user-guide/features/cron
...
- **Telemetry:** sidecar at `~/.et-agent/skills/.usage.json` holds
  per-skill `use_count`, `view_count`, `patch_count`,
  `last_activity_at`, `state`, `pinned`.

Config: `curator.*` (`enabled`, `interval_hours`, `min_idle_hours`,
`stale_after_days`, `archive_after_days`, `backup.*`).
User docs: https://et-agent.nousresearch.com/docs/user-guide/features/curator
...
User docs: https://et-agent.nousresearch.com/docs/user-guide/features/kanban
...
Check logs first:
```bash
grep -i "failed to send\|error" ~/.et-agent/logs/gateway.log | tail -20
```
...
|----------------|----------|
| Config options | `hermes config edit` or [Configuration docs](https://et-agent.nousresearch.com/docs/user-guide/configuration) |
| Available tools | `hermes tools list` or [Tools reference](https://et-agent.nousresearch.com/docs/reference/tools-reference) |
| Slash commands | `/help` in session or [Slash commands reference](https://et-agent.nousresearch.com/docs/reference/slash-commands) |
| Skills catalog | `hermes skills browse` or [Skills catalog](https://et-agent.nousresearch.com/docs/reference/skills-catalog) |
| Provider setup | `hermes model` or [Providers guide](https://et-agent.nousresearch.com/docs/integrations/providers) |
| Platform setup | `hermes gateway setup` or [Messaging docs](https://et-agent.nousresearch.com/docs/user-guide/messaging/) |
| MCP servers | `hermes mcp list` or [MCP guide](https://et-agent.nousresearch.com/docs/user-guide/features/mcp) |
| Profiles | `hermes profile list` or [Profiles docs](https://et-agent.nousresearch.com/docs/user-guide/profiles) |
| Cron jobs | `hermes cron list` or [Cron docs](https://et-agent.nousresearch.com/docs/user-guide/features/cron) |
| Memory | `hermes memory status` or [Memory docs](https://et-agent.nousresearch.com/docs/user-guide/features/memory) |
| Env variables | `hermes config env-path` or [Env vars reference](https://et-agent.nousresearch.com/docs/reference/environment-variables) |
| CLI commands | `hermes --help` or [CLI reference](https://et-agent.nousresearch.com/docs/reference/cli-commands) |
| Gateway logs | `~/.et-agent/logs/gateway.log` |
| Session files | `hermes sessions browse` (reads state.db) |
| Source code | `~/.et-agent/et-agent/` |

---

## Contributor Quick Reference

For occasional contributors and PR authors. Full developer docs: https://et-agent.nousresearch.com/docs/developer-guide/
...
Config: `~/.et-agent/config.yaml` (settings), `~/.et-agent/.env` (API keys) — both under `$HERMES_HOME` when it is set.
...
- Tests auto-redirect `HERMES_HOME` to temp dirs — never touch real `~/.et-agent/`
