...
# $ET_AGENT_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $ET_AGENT_HOME/models_dev_cache.json directly (no network) so this
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$ET_AGENT_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    et-agent is tool-calling-first — every provider path assumes the model
    ...
    *,
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — et-agent
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         when the env var is set in ``~/.et-agent/.env``.
...
#
# Cache strategy:
#   - One JSON file at $ET_AGENT_HOME/provider_models_cache.json
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$ET_AGENT_HOME/auth.json`` and external
