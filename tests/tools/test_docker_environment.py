...
def test_run_command_tags_hermes_agent_label(monkeypatch):
    """Every container et-agent starts must carry the et-agent=1 label
    so the orphan reaper (and external operators) can identify them with a
    single ``docker ps --filter label=et-agent=1`` call. Regression test
    ...
    assert "et-agent=1" in labels, (
        f"et-agent=1 label missing; got labels: {sorted(labels)}"
    )
...
def test_reap_orphan_scopes_to_profile_filter_via_label(monkeypatch):
    ...
    assert "label=et-agent=1" in flat, (
        f"et-agent label filter must also be applied; got: {ps_calls[0][0]}"
    )
...
def test_image_uses_init_entrypoint_detects_s6_init(monkeypatch):
    ...
    monkeypatch.setattr(docker_env.subprocess, "run", _run)
    assert docker_env._image_uses_init_entrypoint("/usr/bin/docker", "et-agent:latest") is True
...
def test_s6_image_skips_docker_init_and_mounts_run_exec(monkeypatch):
    ...
    _make_dummy_env(image="et-agent:latest")
