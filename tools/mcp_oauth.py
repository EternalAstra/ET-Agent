    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "ET-Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses ET_AGENT_HOME so each profile gets its own OAuth tokens.
    Layout: ``ET_AGENT_HOME/mcp-tokens/``
    """
    try:
        from hermes_constants import get_hermes_home
        base = Path(get_hermes_home())
    except ImportError:
        base = Path(os.environ.get("ET_AGENT_HOME", str(Path.home() / ".et-agent")))
...
class HermesTokenStorage:
    ...
    File layout::
        ...
        ET_AGENT_HOME/mcp-tokens/<server_name>.json         -- tokens
        ET_AGENT_HOME/mcp-tokens/<server_name>.client.json   -- client info
        ET_AGENT_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://et-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "ET-Agent")
    scope = cfg.get("scope")
