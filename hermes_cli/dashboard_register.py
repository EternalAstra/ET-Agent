...
resulting ``agent:{id}`` OAuth client ID, and paste it into ``~/.et-agent/.env``
as ``HERMES_DASHBOARD_OAUTH_CLIENT_ID``.

This command:
  1. Resolves a fresh Nous Portal access token from the existing login
     (``~/.et-agent/auth.json``), refreshing it if needed. Fails fast with a
  ...
  3. Writes ``HERMES_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)
     ``HERMES_DASHBOARD_PORTAL_URL`` into ``~/.et-agent/.env`` idempotently.
