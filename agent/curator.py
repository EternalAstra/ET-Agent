...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.et-agent/config.yaml. Tolerates missing file."""
    try:
...
CURATOR_REVIEW_PROMPT = (
    ...
    "into ~/.et-agent/skills/.archive/) is the maximum destructive action. "
    ...
    "~/.et-agent/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.et-agent/logs/curator/``)
    ...
    authored skill data in ``~/.et-agent/skills/``.
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.et-agent/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.et-agent/skills/.archive/` for "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.et-agent/skills/.archive/`. "
    ...
    lines.append("- All archives live under `~/.et-agent/skills/.archive/` and are recoverable by `mv`")
