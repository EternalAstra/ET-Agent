...
Resolution order:
  1. ``ET_AGENT_TIMEZONE`` environment variable
  2. ``timezone`` key in ``~/.et-agent/config.yaml``
...
def _resolve_timezone_name() -> str:
    ...
    tz_env = os.getenv("ET_AGENT_TIMEZONE", "").strip()
    if tz_env:
        return tz_env
...
def reset_cache() -> None:
    ...
    config edit or ``ET_AGENT_TIMEZONE`` update) to force ``get_timezone()`` /
