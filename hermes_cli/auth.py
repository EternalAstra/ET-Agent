"""
Multi-provider authentication system for ET-Agent.

Supports OAuth device code flows (Nous Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.et-agent/auth.json with cross-process file locking.
...
SPOTIFY_DOCS_URL = "https://et-agent.nousresearch.com/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

XAI_OAUTH_DOCS_URL = "https://et-agent.nousresearch.com/docs/guides/xai-grok-oauth"
OAUTH_OVER_SSH_DOCS_URL = "https://et-agent.nousresearch.com/docs/guides/oauth-over-ssh"
...
def _resolve_api_key_provider_secret(
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.et-agent/.env file
...
# =============================================================================
# Auth Store — persistence layer for ~/.et-agent/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_hermes_home() / "auth.json"
    # Seat belt: if pytest is running and ET_AGENT_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch ET_AGENT_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".et-agent" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set ET_AGENT_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom ET_AGENT_HOME that is not a profile).
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.et-agent/auth.json`` even when ET_AGENT_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_hermes_root()`` for a profile-shaped ET_AGENT_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".et-agent" / "auth.json"
            try:
...
def resolve_provider(
    ...
    *,
    ...
    raise AuthError(
        ...
        "OPENAI_API_KEY, etc.) in ~/.et-agent/.env.",
...
#
# Tokens live in ~/.et-agent/auth/google_oauth.json (managed by agent.google_oauth).
...
# Spotify auth — PKCE tokens stored in ~/.et-agent/auth.json
# =============================================================================
...
def _make_xai_callback_handler(expected_path: str) -> tuple[type[BaseHTTPRequestHandler], dict[str, Any]]:
    ...
    class _XAICallbackHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            # ``~/.et-agent/logs/agent.log`` which captures INFO+ unconditionally).
            try:
                logger.info(
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.et-agent/.env, and return it.
    ...
    print("       App name:     anything (e.g. et-agent)")
    ...
    print()
    print("Saved HERMES_SPOTIFY_CLIENT_ID to ~/.et-agent/.env")
    print()
    return raw
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.et-agent/auth.json (not ~/.codex/)
#
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Hermes auth store (~/.et-agent/auth.json).
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to Hermes auth store (~/.et-agent/auth.json)."""
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.et-agent/auth.json
# =============================================================================
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    # ~/.et-agent/auth.json even when ET_AGENT_HOME points at a profile path
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.et-agent/auth.json``. A single MITM during initial
...
# It is OUTSIDE any named profile's ET_AGENT_HOME so named profiles (which
...
def _nous_shared_auth_dir() -> Path:
    ...
    Linux/macOS classic installs land at ``~/.et-agent/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\hermes\\shared\\``, and
    Docker / custom ``ET_AGENT_HOME`` deployments at
    ``<ET_AGENT_HOME>/shared/``. Sits outside any named profile so all
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No ET_AGENT_HOME yet (pre-setup): fall through without locking.
        yield
        return
    ...
    *,
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "~/.et-agent/auth.json) called POST /api/oauth/token with Hermes's "
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.et-agent/auth.json."""
...
def _xai_oauth_build_authorize_url(
    *,
    ...
    # `referrer=et-agent` lets xAI attribute Hermes-originated logins
    ...
    *,
    ...
    if not code_verifier:
        raise AuthError(
            ...
            "https://github.com/NousResearch/et-agent/issues/26990.",
            provider="xai-oauth",
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Hermes auth store (~/.et-agent/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
