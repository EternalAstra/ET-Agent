...
AUTHOR_MAP = {
    ...
    "274096618+et-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (ET_AGENT_HOME inject)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level ET_AGENT_HOME state files)
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"et-agent[acp]=={semver}"
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/et-agent",
    ...
    lines.append(f"# ET-Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for ET-Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="ET-Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  ET-Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"ET-Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"ET-Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'ET-Agent v{new_version} ({calver_date})' "
