...
- Subscriptions persist to `~/.et-agent/webhook_subscriptions.json`

## How It Works

1. `hermes webhook subscribe` writes to `~/.et-agent/webhook_subscriptions.json`
...
3. **Check gateway logs:** `grep webhook ~/.et-agent/logs/gateway.log | tail -20`
