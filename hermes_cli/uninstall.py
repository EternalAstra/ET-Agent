"""
ET-Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.et-agent/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing et-agent or hermes PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# ET-Agent" comment and following line
                if '# ET-Agent' in line or '# et-agent' in line:
                    skip_next = True
                    continue
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                ...
                if 'hermes_cli' in content or 'et-agent' in content:
                    wrapper.unlink()
...
#
#   1. Sets User-scope env vars ``ET_AGENT_HOME`` and ``HERMES_GIT_BASH_PATH``
...
def _hermes_path_markers(hermes_home: Path) -> list[str]:
    ...
    # all get swept.  Also match the bare et-agent install dir.
    markers = [root + "\\et-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if ET_AGENT_HOME was customised to somewhere else — find-and-nuke
...
def remove_hermes_env_vars_windows() -> list[str]:
    """Delete ET_AGENT_HOME and HERMES_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("ET_AGENT_HOME", "HERMES_GIT_BASH_PATH"):
                try:
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ET_AGENT_HOME directory.
    ...
    current ET_AGENT_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    # 3. Wipe the profile's ET_AGENT_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    userData dir — nothing under ``$ET_AGENT_HOME`` config/sessions/.env, and
    ...
    if agent_is_installed(hermes_home):
        ...
        print(f"  • The Hermes agent at {hermes_home / 'et-agent'}")
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.et-agent/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.et-agent/ for future reinstall
    """
    project_root = get_project_root()
    hermes_home = get_hermes_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie ET_AGENT_HOMEs
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ ET-Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    # their alias wrappers, and wiping their ET_AGENT_HOME dirs. Otherwise
...
def _perform_uninstall(
    *,
    ...
    wipe ``$ET_AGENT_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        log_info("Removing ET_AGENT_HOME / HERMES_GIT_BASH_PATH User env vars...")
    ...
    #     but the packaged app + Electron userData live OUTSIDE ET_AGENT_HOME and
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.et-agent/, just remove the et-agent subdir
    ...
    #     under ET_AGENT_HOME but they're install tooling, not config — safe to
    ...
    # 5. Optionally remove ~/.et-agent/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile ET_AGENT_HOME dirs live under
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://et-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://et-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using ET-Agent! ⚕")
    print()
