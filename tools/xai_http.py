...
def has_xai_credentials() -> bool:
    ...
    2. ``~/.et-agent/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.et-agent/.env`` first, then ``os.environ``.
...
def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    ...
    stored in ``~/.et-agent/.env`` (the standard Hermes location) are honored —
