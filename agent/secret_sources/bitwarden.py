...
so they don't have to live in plaintext in ``~/.et-agent/.env``.
...
* The access token is stored in ``~/.et-agent/.env`` as
...
# never the access token. It's plaintext-equivalent to ~/.et-agent/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    ...
    to `$ET_AGENT_HOME` / `~/.hermes` keeps direct callers working too.
    """
    if home_path is None:
        home_path = Path(os.getenv("ET_AGENT_HOME", Path.home() / ".et-agent"))
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "et-agent"})
    try:
