"""On-demand supply-chain audit for ET-Agent installs.
...
2. Python deps declared by user-installed plugins under ``~/.et-agent/plugins``
...
def _discover_plugins(hermes_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.et-agent/plugins``.
