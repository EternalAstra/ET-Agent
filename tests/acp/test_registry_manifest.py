"""Tests for ACP Registry metadata bundled with ET-Agent."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "et-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "ET-Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/NousResearch/et-agent"
    assert data["website"].startswith("https://et-agent.nousresearch.com/")
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"et-agent[acp]=={data['version']}"
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"et-agent[acp]=={_pyproject_version()}"
    )
