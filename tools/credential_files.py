...
def register_credential_file(
    ...
    *relative_path* is relative to ``ET_AGENT_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside ET_AGENT_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    hermes_home = _resolve_hermes_home()

    # Reject absolute paths — they bypass the ET_AGENT_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to ET_AGENT_HOME)",
            relative_path,
        )
        return False
    ...
    # that traversal like ``../. ssh/id_rsa`` cannot escape ET_AGENT_HOME.
