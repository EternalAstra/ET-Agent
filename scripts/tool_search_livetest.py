#!/usr/bin/env python3
"""Live test harness for ET-Agent's Tool Search feature.
...
ORIGINAL_HOME = os.environ.get("ET_AGENT_HOME")
ORIGINAL_AUTH = Path.home() / ".et-agent" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.et-agent/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.et-agent/.env`` so
    ...
    real_env_file = Path.home() / ".et-agent" / ".env"
    if real_env_file.exists():
        ...
        load_hermes_dotenv(hermes_home=str(Path.home() / ".et-agent"))
...
def reset_module_state():
    """Drop cached modules so the new ET_AGENT_HOME takes effect."""
...
def main():
    ...
    # Restore original ET_AGENT_HOME
    if ORIGINAL_HOME is not None:
        os.environ["ET_AGENT_HOME"] = ORIGINAL_HOME
    else:
        os.environ.pop("ET_AGENT_HOME", None)
