...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.et-agent/skills/`` are silently skipped.
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against ET_AGENT_HOME, not cwd
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.et-agent/skills/`` first, then external.
