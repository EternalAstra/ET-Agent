...
Each profile is a fully independent ET_AGENT_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.et-agent/profiles/<name>/`` by default.
...
#
# Rationale per item:
#   et-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "et-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the hermes root, NOT to the current ET_AGENT_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ET_AGENT_HOME points outside
    ``~/.hermes``, profiles live under ``ET_AGENT_HOME/profiles/`` so
...
def _get_default_hermes_home() -> Path:
    """Return the default (pre-profile) ET_AGENT_HOME path.

    In standard deployments this is ``~/.hermes``.
    In Docker/custom deployments where ET_AGENT_HOME is outside ``~/.hermes``
    (e.g. ``/opt/data``), returns ET_AGENT_HOME directly.
    """
...
def validate_profile_name(name: str) -> None:
    ...
    collisions (a ``hermes`` profile inside ``~/.et-agent/``) or get refused
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its ET_AGENT_HOME directory."""
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own ET_AGENT_HOME.
...
def create_profile(
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.et-agent/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ET_AGENT_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ET_AGENT_HOME": str(profile_dir)},
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``ET_AGENT_HOME`` and binds the port resolved by ``gateway/config.py``
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ET_AGENT_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("ET_AGENT_HOME")
    try:
        os.environ["ET_AGENT_HOME"] = str(profile_dir)
        ...
        if old_home is not None:
            os.environ["ET_AGENT_HOME"] = old_home
        elif "ET_AGENT_HOME" in os.environ:
            del os.environ["ET_AGENT_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.et-agent/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ET_AGENT_HOME.

    Returns ``"default"`` if ET_AGENT_HOME is not set or points to ``~/.hermes``.
    Returns the profile name if ET_AGENT_HOME points into ``~/.et-agent/profiles/<name>``.
    Returns ``"custom"`` if ET_AGENT_HOME is set to an unrecognized path.
    """
    from hermes_constants import get_hermes_home
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        ...
        # directory name is ".et-agent", not "default".  We stage a clean copy
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ET_AGENT_HOME path string.

    Called early in the CLI entry point, before any hermes modules
    are imported, to set the ET_AGENT_HOME environment variable.
    """
    canon = normalize_profile_name(profile_name)
    validate_profile_name(canon)
    profile_dir = get_profile_dir(canon)
