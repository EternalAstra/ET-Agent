...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``hermes desktop`` / ``hermes gui``)
     Built inside the agent checkout under ``$ET_AGENT_HOME/et-agent/``:
       ...
       - ``$ET_AGENT_HOME/desktop-build-stamp.json`` (the build freshness stamp)
...
the app name ("Hermes"), separate from ``$ET_AGENT_HOME``:
...
def source_built_gui_artifacts(hermes_home: Path) -> "list[Path]":
    ...
    Python agent runs from ``et-agent/`` source + ``venv/`` and never
...
def agent_is_installed(hermes_home: Path) -> bool:
    """Return True when a usable Python agent install exists under ET_AGENT_HOME.
...
def uninstall_gui(hermes_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``et-agent/hermes_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$ET_AGENT_HOME``.
