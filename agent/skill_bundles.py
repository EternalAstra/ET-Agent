...
Storage
-------
Bundles live in ``~/.et-agent/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``ET_AGENT_HOME``). Each file looks like::
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under ET_AGENT_HOME.

    Honors ``HERMES_BUNDLES_DIR`` for tests; falls back to
    ``<ET_AGENT_HOME>/skill-bundles``.
    """
