...
def _find_bash() -> str:
    ...
    raise RuntimeError(
        "Git Bash not found. ET-Agent requires Git for Windows on Windows.\n"
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``ET_AGENT_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under ET_AGENT_HOME.  Using
