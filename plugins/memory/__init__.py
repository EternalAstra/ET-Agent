...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with et-agent)
2. User-installed providers: ``$ET_AGENT_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$ET_AGENT_HOME/plugins/`` or None if unavailable."""
    try:
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$ET_AGENT_HOME/plugins/<name>/``) directories.  Bundled takes
