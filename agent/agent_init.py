...
def init_agent(
    ...
    Args:
        ...
        load_soul_identity (bool): If True, still use ~/.et-agent/SOUL.md as the primary
    ...
    # both live under ~/.et-agent/logs/.  Idempotent, so gateway mode
    ...
    # Session logs go into ~/.et-agent/sessions/ alongside gateway sessions
    hermes_home = get_hermes_home()
    agent.logs_dir = hermes_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.et-agent/sessions/session_{sid}.json)
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by ET-Agent.  Choose a model with at least "
