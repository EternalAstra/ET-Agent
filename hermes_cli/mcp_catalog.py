...
Catalog policy:
- Entries are added only by merging a PR into et-agent. Presence in the
...
- Secrets prompted at install time go to ``~/.et-agent/.env`` (the
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.et-agent/mcp-installs/<name>`` and run
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.et-agent/.env via save_env_value()."""
