...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.et-agent/config.yaml``. Hermes
...
Configuration is loaded from ~/.et-agent/config.yaml under the 'tts:' key.
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.et-agent/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.et-agent/config.yaml.
...
def _get_piper_voices_dir() -> Path:
    ...
    Resolves to ``~/.et-agent/cache/piper-voices/`` under the active
    ET_AGENT_HOME so voice downloads follow profile boundaries.
    """
    from hermes_constants import get_hermes_dir
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.et-agent/config.yaml (tts: section).
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'et-agent[mistral]'"
                }, ensure_ascii=False)
