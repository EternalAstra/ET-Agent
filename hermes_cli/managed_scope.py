...
user's ``~/.et-agent/config.yaml`` and ``~/.et-agent/.env`` on a per-leaf-key basis.

This is DISTINCT from ``hermes_cli.config.is_managed()`` / ``ET_AGENT_MANAGED``,
...
def _under_pytest() -> bool:
    ...
    that exercise managed scope set ``ET_AGENT_MANAGED_DIR`` explicitly, which is
...
def get_managed_dir() -> Optional[Path]:
    ...
    Resolution (highest priority first):
      1. ``$ET_AGENT_MANAGED_DIR`` — deployment/bootstrap path override (IT-only;
    ...
    """
    override = os.environ.get("ET_AGENT_MANAGED_DIR", "").strip()
    if override:
        p = Path(override)
