...
deployments where ``ET_AGENT_HOME`` points outside ``~/.hermes`` (e.g.
``/opt/hermes``), ``<root>`` is ``ET_AGENT_HOME``. Legacy env-var
...
# enough that kebab-case names like ``atm10-server`` or ``et-agent``
...
def kanban_home() -> Path:
    ...
    2. ``get_default_hermes_root()``, which already returns ``<root>``
       when ``ET_AGENT_HOME`` is ``<root>/profiles/<name>``, and returns
       ``ET_AGENT_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``ET_AGENT_HOME`` would silently fork the board per profile,
...
def _kanban_worker_skill_available(hermes_home: Optional[str]) -> bool:
    ...
    # An unset ET_AGENT_HOME means the worker falls back to the default root
    # home (``~/.hermes``), which ships the bundled skill.
    base = _Path(hermes_home) if hermes_home else (_Path.home() / ".et-agent")
...
def _resolve_worker_cli_toolsets(hermes_home: Optional[str]) -> Optional[list[str]]:
    ...
    try:
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for ET_AGENT_HOME=%r (%s)",
            hermes_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    # Inject ET_AGENT_HOME so the worker reads the profile-scoped config.yaml
    ...
    # If ET_AGENT_HOME is absent from the child's env, get_hermes_home() falls
    # back to Path.home() / ".et-agent" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from hermes_cli.profiles import resolve_profile_env
    try:
        env["ET_AGENT_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via HERMES_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # ET_AGENT_HOME never had profiles created.
        pass
    if task.tenant:
    ...
    # rewrites ET_AGENT_HOME), its kanban paths still match the
    ...
    cmd = [
        *_resolve_hermes_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped ET_AGENT_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("ET_AGENT_HOME")):
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("ET_AGENT_HOME"))
    if worker_toolsets:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Install ET-Agent or activate its venv before running the kanban dispatcher."
        )
