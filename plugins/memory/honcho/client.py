...
Resolution order for config file:
  1. $ET_AGENT_HOME/honcho.json  (instance-local, enables isolated Hermes instances)
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $ET_AGENT_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.et-agent/honcho.json          (default profile — shared host blocks live here)
...
class HonchoClientConfig:
    ...
    def from_global_config(
        ...
        Resolution: $ET_AGENT_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
