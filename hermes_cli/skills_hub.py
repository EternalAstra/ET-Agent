...
def _existing_categories() -> List[str]:
    """Return sorted subdirectory names under ``~/.et-agent/skills/`` that look
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print(
            "[bold]Category[/] [dim](optional — press Enter to install flat at ~/.et-agent/skills/<name>/)[/]"
        )
    try:
    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            c.print(Panel(
                ...
                "It ships with et-agent but is not activated by default.\n"
...
def do_list(source_filter: str = "all",
    ...
    ``skills.disabled`` list because ``-p`` swaps ``ET_AGENT_HOME`` at process
