...
def _hermes_home_path() -> Path:
    """Resolve the active ET_AGENT_HOME (profile-aware) without circular imports."""
    try:
        from hermes_constants import get_hermes_home  # local import to avoid cycles
        return get_hermes_home()
    except Exception:
        return Path(os.path.expanduser("~/.et-agent"))
...
def get_read_block_error(path: str) -> Optional[str]:
    ...
    Three categories are blocked:
      ...
      * Internal Hermes cache files under ``ET_AGENT_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under ET_AGENT_HOME and the global Hermes
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.et-agent/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active ET_AGENT_HOME (profile-aware) AND the global
    # Hermes root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (ET_AGENT_HOME points at
...
#
# Hermes profiles are separate ET_AGENT_HOME dirs under
...
# edited skills under both ``~/.et-agent/profiles/hermes-security/skills/``
# AND ``~/.et-agent/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under ET_AGENT_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from ET_AGENT_HOME.

    ``~/.et-agent``              -> ``"default"``
    ``~/.et-agent/profiles/X``  -> ``"X"``
...
#
#   <ET_AGENT_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.et-agent/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.et-agent/…`` segment and warns
...
# (the agent's view inside the container is plain ``/root/.et-agent/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.et-agent`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.et-agent/…`` and returns the
    ...
    for i, part in enumerate(parts):
        ...
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".et-agent":
            return i + 4
    return None
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    ...
    Otherwise returns a dict with:
      ...
      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.et-agent``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.et-agent```` (what the
    ...
    ET_AGENT_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"under the real ET_AGENT_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.et-agent/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.et-agent/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host ET_AGENT_HOME)
    """
...
def get_container_mirror_warning(
    ...
    return (
        ...
        f"{info['inner_path']!r} under the real ET_AGENT_HOME. Use the "
