#!/usr/bin/env python3
"""
ET-Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the ET-Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills et-agent-dev,github-auth
...
def _load_prefill_messages(file_path: str) -> List[Dict[str, Any]]:
    ...
    Relative paths are resolved from ~/.et-agent/.
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.et-agent/config.yaml (user config - preferred)
    ...
    the user config at ``~/.et-agent/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({ET_AGENT_HOME}/config.yaml)
...
# Initialize centralized logging early — agent.log + errors.log in ~/.et-agent/logs/.
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        ...
        agent_name = _skin.get_branding("agent_name", "ET-Agent") if _skin else "ET-Agent"
    ...
    if os.environ.get("HERMES_FAST_STARTUP_BANNER") == "1":
        ...
        version_line = f"ET-Agent v{_version} ({_release_date})"
    else:
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.et-agent/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.et-agent/config.yaml on first use)
...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin):
    """
    Interactive CLI for the ET-Agent.
    ...
    def __init__(
        ...
        # it's shared across all Hermes processes for this ET_AGENT_HOME.
        # Never blocks startup on failure.
        _run_state_db_auto_maintenance(self._session_db)

        # Opportunistic shadow-repo cleanup — deletes orphan/stale
        # checkpoint repos under ~/.et-agent/checkpoints/.  Opt-in via
    ...
    def show_banner(self):
        ...
        if is_nous_hermes_non_agentic(model_name):
            ...
            self._console_print(
                ...
                "designed for use with ET-Agent.[/]"
            )
            self._console_print(
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.et-agent/images/ and appends the path to
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.et-agent/sessions/saved/.
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.et-agent/skills/ and queue a note for the
        next user turn.
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard ET_AGENT_HOME, so no -p needed.
            try:
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to ET-Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to ET-Agent! Type your message or /help for commands."
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from hermes_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "ET-Agent")
...
def main(
    ...
    """
    ET-Agent CLI - Interactive AI Assistant
