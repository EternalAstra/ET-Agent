...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    real gateway on the same ET_AGENT_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
app = FastAPI(title="ET-Agent", version=__version__, lifespan=_lifespan)
...
def _default_hermes_root_is_opt_data() -> bool:
    raw = os.environ.get("ET_AGENT_HOME", "").strip()
    if not raw:
        return False
    try:
...
# streamed to a per-action log file under ``~/.et-agent/logs/<action>.log`` so
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.et-agent/config.yaml`` under ``tts.``.
    """
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.et-agent/config.yaml`` — applies to **new** sessions only.
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    ...
    "email": {
        ...
        "docs_url": "https://et-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "weixin": {
        ...
        "docs_url": "https://et-agent.nousresearch.com/docs/user-guide/messaging/weixin/",
    ...
    "webhook": {
        ...
        "docs_url": "https://et-agent.nousresearch.com/docs/user-guide/messaging/webhooks/",
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.et-agent.nousresearch.com"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "ET-Agent").strip() or "ET-Agent"
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's ET_AGENT_HOME.
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.et-agent/.anthropic_oauth.json`` — Hermes-managed PKCE flow (what
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "docs_url": "https://et-agent.nousresearch.com/docs/guides/xai-grok-oauth",
...
#          → persists to ~/.et-agent/.anthropic_oauth.json AND credential pool
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, ET_AGENT_HOME)."""
    from hermes_cli import profiles as profiles_mod
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process ET_AGENT_HOME at import time. The dashboard is a single
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    def _probe_scoped():
        ...
        # ET_AGENT_HOME override (see mcp_tool._wrap_with_home_override), so
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds ET_AGENT_HOME-derived paths in the child.
...
class ProfileCreate(BaseModel):
    ...
    # because skills_hub.SKILLS_DIR is import-time-bound and the ET_AGENT_HOME
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local ET_AGENT_HOME override so the write lands in the target
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local ET_AGENT_HOME override (same mechanism as
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the ET_AGENT_HOME override. Returns the
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's ET_AGENT_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    the running dashboard/gateway is scoped to (derived from ET_AGENT_HOME).
    """
    from hermes_cli import profiles as profiles_mod
    try:
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the ET_AGENT_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
def _profile_scope(profile: Optional[str]):
    ...
    imported the modules before a ET_AGENT_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.et-agent/.env`` via ``save_env_value`` —
...
async def run_toolset_post_setup(
    ...
    write per-profile state must see the same ET_AGENT_HOME the rest of the
...
def _resolve_chat_argv(
    ...
    ``ET_AGENT_HOME`` at the profile dir in the child env. Every spawned
    ...
    if profile_dir is not None:
        env["ET_AGENT_HOME"] = str(profile_dir)
    ...
    # inherits the profile ET_AGENT_HOME set above.
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.et-agent/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.et-agent/dashboard-themes/*.yaml for user-created themes.
...
async def get_dashboard_themes():
    ...
    from `~/.et-agent/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    1. User plugins:    ~/.et-agent/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.hermes/plugins/  (only if ET_AGENT_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()
    ...
    if env_var_enabled("ET_AGENT_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".et-agent" / "plugins", "project"))
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.et-agent/plugins/ if you trust it)",
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # Each provider plugin that ships with ET-Agent exposes a
