...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with et-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.et-agent/plugins/<name>/``
3. **Project plugins** – ``./.hermes/plugins/<name>/`` (opt-in via
   ``ET_AGENT_ENABLE_PROJECT_PLUGINS``)
...
def get_bundled_plugins_dir() -> Path:
    ...
    Honours ``ET_AGENT_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("ET_AGENT_BUNDLED_PLUGINS")
    if env_override:
...
# stderr in addition to ~/.et-agent/logs/agent.log. Aimed at plugin authors
...
class PluginManifest:
    ...
    #              in ~/.et-agent/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
...
class PluginContext:
    ...
    def register_skill(
        ...
        ``~/.et-agent/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        # 2. User plugins (~/.et-agent/plugins/)
        ...
        if _env_enabled("ET_AGENT_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".et-agent" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set ET_AGENT_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
