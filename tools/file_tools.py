...
def _get_hermes_config_resolved() -> str | None:
    ...
    try:
        ...
        try:
            _hermes_config_resolved = str(Path("~/.et-agent/config.yaml").expanduser().resolve())
        except Exception:
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if hermes_config and (resolved == hermes_config or normalized == hermes_config):
        return (
            ...
            "Edit ~/.et-agent/config.yaml directly or use 'hermes config' instead."
        )
    return None
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    * container-mirror (#32049 follow-up) — writes from inside a Docker
    ...
    # in a session that cd'd into ``~/.et-agent/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # and block credential stores under ET_AGENT_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == ET_AGENT_HOME (e.g. "auth.json") still hits the
