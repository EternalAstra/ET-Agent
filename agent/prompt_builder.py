...
DEFAULT_AGENT_IDENTITY = (
    "You are ET-Agent, an intelligent AI assistant created by Nous Research. "
...
HERMES_AGENT_HELP_GUIDANCE = (
    "You run on ET-Agent (by Nous Research). When the user needs help with "
    "Hermes itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://et-agent.nousresearch.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `et-agent` skill with skill_view(name='et-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.et-agent/kanban.db`. Your task id is in "
...
#
# The et-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to et-agent's Python
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.et-agent/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot ET-Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `et-agent` skill "
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from ET_AGENT_HOME and return its content, or None.
    ...
    try:
        ...
        logger.debug("Could not ensure ET_AGENT_HOME before loading SOUL.md: %s", e)
...
def build_context_files_prompt(
    ...
    SOUL.md from ET_AGENT_HOME is independent and always included when present.
    ...
    # SOUL.md from ET_AGENT_HOME only — skip when already loaded as identity
    if not skip_soul:
