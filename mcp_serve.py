...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ET_AGENT_HOME."""
    try:
        from hermes_constants import get_hermes_home
        return get_hermes_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ET_AGENT_HOME", Path.home() / ".et-agent")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        ...
        directory_file = Path(
            os.environ.get("ET_AGENT_HOME", Path.home() / ".et-agent")
...
class EventBridge:
    ...
    def _poll_once(self, db):
        ...
        try:
            ...
            db_file = Path(os.environ.get("ET_AGENT_HOME", Path.home() / ".et-agent")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    ...
    mcp = FastMCP(
        "hermes",
        instructions=(
            "ET-Agent messaging bridge. Use these tools to interact with "
