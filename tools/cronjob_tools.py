"""
Cron job management tools for ET-Agent.
...
#      saying "cat ~/.et-agent/.env" or "rm -rf /". `_scan_cron_prompt()` runs
...
#      `et-agent-dev` skill contains a security postmortem mentioning
#      `cat ~/.et-agent/.env`, which tripped `read_secrets` and silently
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within ET_AGENT_HOME/scripts/.
    ...
    # Only relative paths within ~/.et-agent/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.et-agent/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.et-agent/scripts/ and use just the filename."
        )
