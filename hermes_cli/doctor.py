...
Diagnoses issues with ET-Agent setup.
"""
...
_DHH = display_hermes_home()  # user-facing display path (e.g. ~/.hermes or ~/.et-agent/profiles/coder)

# Load environment variables from ~/.et-agent/.env so API key checks work
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.et-agent/.env contains provider auth/base URL settings."""
...
def managed_scope_check() -> None:
    ...
    resolved from the ET_AGENT_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("ET_AGENT_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via ET_AGENT_MANAGED_DIR={managed_dir}")
...
def run_doctor(args):
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.et-agent/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    managed_scope_check()
    # Check ~/.et-agent/.env (primary location for user config)
    env_path = ET_AGENT_HOME / '.env'
    if env_path.exists():
    ...
    # Check ~/.et-agent/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = ET_AGENT_HOME / 'config.yaml'
                ...
                try:
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.et-agent/.env or run 'hermes setup')",
                            (
    ...
    _section("Skills Hub")
    hub_dir = ET_AGENT_HOME / "skills" / ".hub"
    if hub_dir.exists():
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = ET_AGENT_HOME / "config.yaml"
