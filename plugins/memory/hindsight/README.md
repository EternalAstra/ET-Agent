...
echo "HINDSIGHT_API_KEY=your-key" >> ~/.et-agent/.env
```
...
Daemon startup logs: `~/.et-agent/logs/hindsight-embed.log`
...
Config file: `~/.et-agent/hindsight/config.json`
...
| `retain_context` | `conversation between ET-Agent and the User` | Context label for retained memories |
...
The LLM API key is stored in `~/.et-agent/.env` as `HINDSIGHT_LLM_API_KEY`.
