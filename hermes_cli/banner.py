...
_UPSTREAM_REPO_URL = "https://github.com/NousResearch/et-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/nousresearch/et-agent"
...
def _fetch_pypi_latest(package: str = "et-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    ...
    if embedded_rev:
        ...
        # $ET_AGENT_HOME/et-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = hermes_home / "et-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    ...
    because ``$ET_AGENT_HOME/et-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
_RELEASE_URL_BASE = "https://github.com/NousResearch/et-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    canonical NousResearch/et-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"ET-Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    # Pip-install warning — `pip install et-agent` is not the supported
