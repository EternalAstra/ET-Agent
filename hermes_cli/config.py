"""
Configuration management for ET-Agent.

Config files are stored in ~/.et-agent/ for easy access:
- ~/.et-agent/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.et-agent/.env         - API keys and secrets
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.et-agent/config.yaml`` causes ``load_config()``
...
# * ``ET_AGENT_HOME`` / ``HERMES_PROFILE`` / ``HERMES_CONFIG`` /
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    "ET_AGENT_HOME", "HERMES_PROFILE", "HERMES_CONFIG", "HERMES_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "(ET_AGENT_HOME, HERMES_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.et-agent/.env directly."
        )
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ET_AGENT_MANAGED", "").strip()
    if raw:
...
def is_managed() -> bool:
    ...
    Two signals: the ET_AGENT_MANAGED env var (set by the systemd service),
    or a .managed marker file in ET_AGENT_HOME (set by the NixOS activation
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    interpreter, NOT of ``$ET_AGENT_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    2. Legacy home-scoped stamp ``$ET_AGENT_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. ET_AGENT_MANAGED env / .managed marker (NixOS, Homebrew)
    ...
    ``$ET_AGENT_HOME`` is a shared DATA directory — the Docker docs deliberately
    ...
    containerised gateway and a host install share one ``$ET_AGENT_HOME``, a
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/et-agent`` image bakes a ``docker``
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $ET_AGENT_HOME.
    try:
    ...
    #    host install whose shared $ET_AGENT_HOME was stamped by a co-located
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$ET_AGENT_HOME``, so that two installs sharing one data directory
...
def is_uv_tool_install() -> bool:
    ...
    ``uv tool install et-agent`` places the install at
    ``.../uv/tools/et-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``et-agent`` happens to be uv-tool-installed on the machine while
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/et-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade et-agent"
    if method == "docker":
        return "docker pull nousresearch/et-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade et-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade et-agent"
        return "pip install --upgrade et-agent"
...
ET-Agent runs as a published image (nousresearch/et-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/et-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate et-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/et-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/et-agent/tags
  • Your config and session history live under ``$ET_AGENT_HOME`` (``/opt/data``
...
def format_managed_message(action: str = "modify this Hermes installation") -> str:
    ...
    raw = os.getenv("ET_AGENT_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Hermes installation is managed by NixOS "
            f"(ET_AGENT_MANAGED={env_hint}).\n"
            "Edit services.et-agent.settings in your configuration.nix and run:\n"
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ET_AGENT_HOME/.container-mode.
    ...
    container_name = info.get("container_name", "et-agent")
...
def _resolve_hermes_uid_gid() -> tuple[Optional[int], Optional[int]]:
    ...
    ownership. The entrypoint chowns the top-level ET_AGENT_HOME once, but
...
def _secure_dir(path):
    ...
    The mode can be overridden via the ET_AGENT_HOME_MODE environment variable
    (e.g. ET_AGENT_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ET_AGENT_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ET_AGENT_HOME_MODE", "").strip()
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ET_AGENT_HOME if the user doesn't have one yet."""
...
def _ensure_hermes_home_managed(home: Path):
    ...
    if not home.is_dir():
        raise RuntimeError(
            f"ET_AGENT_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        #             ET_AGENT_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force ET_AGENT_HOME/home when it exists (old strict
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.et-agent/checkpoints/`` size (MB).  When
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.et-agent/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.et-agent/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # always goes to ~/.et-agent/skills/.
    "skills": {
    ...
    "curator": {
        ...
        # skipped), snapshot ~/.et-agent/skills/ into
        # ~/.et-agent/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *ET-Agent*" header.
    ...
    # stored approval from ~/.et-agent/shell-hooks-allowlist.json.
    ...
    "cron": {
        ...
        # $ET_AGENT_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
    ...
    "code_execution": {
        ...
        #     et-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.et-agent/logs/.
    ...
    # update model picker lists without shipping a et-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://et-agent.nousresearch.com/docs/api/model-catalog.json",
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.et-agent/.env,
        ...
        # cache (~/.et-agent/cache/{documents,images,audio,video,screenshots})
    ...
    # Session storage — controls automatic cleanup of ~/.et-agent/state.db.
    ...
    "sessions": {
        ...
        # rewrites ``~/.et-agent/sessions/session_{sid}.json`` on every turn
    ...
    "updates": {
        # Run a full ``hermes backup``-style zip of ET_AGENT_HOME before every
        # ``hermes update``.  Backups land in ``<ET_AGENT_HOME>/backups/`` and
    ...
    "lsp": {
        ...
        #              ``<ET_AGENT_HOME>/lsp/bin/`` on first use.
    ...
    # rather than storing them in ~/.et-agent/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.et-agent/.env (or your shell) and never in config.yaml.
            ...
            # ~/.et-agent/bin/ on first use.  When False you must install
...
OPTIONAL_ENV_VARS = {
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'et-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("ET_AGENT_HOME", "~/.et-agent")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("ET_AGENT_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from ET_AGENT_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$ET_AGENT_HOME/plugins/`` for currently installed user plugins.
    ...
    #   3. Creates `~/.et-agent/logs/curator/` if missing (belt-and-suspenders
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.et-agent/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.et-agent/config.yaml.
    ...
    (which change ``ET_AGENT_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.et-agent/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.et-agent/.env.
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.et-agent/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.et-agent/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.et-agent/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.et-agent/.env into os.environ. Returns count of vars updated.
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.et-agent/.env or environment."""
