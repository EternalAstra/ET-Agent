...
# via shell expansions like $HOME or $ET_AGENT_HOME, or by the resolved absolute
# active profile home path such as /home/hermes/.hermes/config.yaml. The
# resolved-absolute form is folded into the ~/.et-agent/ patterns at detection
...
# go stale when ET_AGENT_HOME is set after this module is imported, e.g. under the
...
# ~/.et-agent/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _HERMES_ENV_PATH; matches the ET_AGENT_HOME override form as
# well as ~/.et-agent/.
_HERMES_CONFIG_PATH = (
    r'(?:~\/\.hermes/|'
    r'(?:\$home|\$\{home\})/\.hermes/|'
...
DANGEROUS_PATTERNS = [
    ...
    # ~/.et-agent/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Hermes-managed security file (~/.et-agent/config.yaml or
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/.et-agent/ form so the Hermes config/env patterns catch it. In Docker and
    ...
    # ~, $HOME, or $ET_AGENT_HOME. Done at detection time (not via an import-time
    # pattern snapshot) so it tracks the live ET_AGENT_HOME even when that is set
...
def _rewrite_resolved_hermes_home(command: str) -> str:
    """Rewrite the resolved absolute Hermes home prefix to ``~/.et-agent/``.

    Resolves the active ``ET_AGENT_HOME`` at call time (and its symlink-resolved
    form) and replaces an occurrence of ``<home>/`` in *command* with
    ``~/.et-agent/`` so the static ``_HERMES_CONFIG_PATH`` / ``_HERMES_ENV_PATH``
    ...
    for path in candidates:
        if not path or path in seen:
            continue
        seen.add(path)
        # Guard against a degenerate ET_AGENT_HOME (e.g. "/" or "") rewriting
        ...
        if not normalized.startswith("/") or normalized.count("/") < 2:
            continue
        command = command.replace(normalized + "/", "~/.et-agent/")
    return command
