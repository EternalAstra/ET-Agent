...
def _make_hermes_tree(root: Path) -> None:
    ...
    # et-agent repo (should be EXCLUDED)
    (root / "et-agent").mkdir(exist_ok=True)
    (root / "et-agent" / "run_agent.py").write_text("# big file\n")
    (root / "et-agent" / ".git").mkdir()
    (root / "et-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_hermes_agent(self):
        ...
        assert _should_exclude(Path("et-agent/run_agent.py"))
        assert _should_exclude(Path("et-agent/.git/HEAD"))
    ...
    def test_includes_nested_hermes_agent_in_skills(self):
        """skills/autonomous-ai-agents/et-agent/ must NOT be excluded —
        only the root-level et-agent/ repo is skipped."""
        from hermes_cli.backup import _should_exclude
        assert not _should_exclude(Path("skills/autonomous-ai-agents/et-agent/SKILL.md"))
        assert not _should_exclude(Path("skills/autonomous-ai-agents/et-agent/sub/item.txt"))
...
class TestBackup:
    ...
    def test_excludes_hermes_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include et-agent/ directory."""
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
        _make_hermes_tree(hermes_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "et-agent" in n]
            assert agent_files == [], f"et-agent files leaked into backup: {agent_files}"
    ...
    def test_includes_nested_hermes_agent_in_skills(self, tmp_path, monkeypatch):
        """Backup includes skills/.../et-agent/ but NOT root et-agent/."""
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
        _make_hermes_tree(hermes_home)

        # Add a nested et-agent directory inside skills (like the real layout)
        nested = hermes_home / "skills" / "autonomous-ai-agents" / "et-agent"
        nested.mkdir(parents=True)
        (nested / "SKILL.md").write_text("# ET-Agent Skill\n")
        (nested / "sub").mkdir()
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            # Root et-agent must be excluded
            root_agent = [n for n in names if n.startswith("et-agent/")]
            assert root_agent == [], f"root et-agent leaked: {root_agent}"
            # Nested skill et-agent must be included
            assert "skills/autonomous-ai-agents/et-agent/SKILL.md" in names
            assert "skills/autonomous-ai-agents/et-agent/sub/item.txt" in names
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # et-agent should NOT be present
        assert not (dst_home / "et-agent").exists()
...
class TestPreUpdateBackup:
    ...
    def test_backup_contents_match_full_backup(self, hermes_home):
        ...
        assert "skills/my-skill/SKILL.md" in names
        assert "profiles/coder/config.yaml" in names
        # et-agent repo excluded
        assert not any(n.startswith("et-agent/") for n in names)
