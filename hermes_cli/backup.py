...
`hermes backup` creates a zip archive of the entire ~/.et-agent/ directory
(excluding the et-agent repo and transient files).
...
# ``et-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/et-agent/``
...
# ET_AGENT_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "et-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under ET_AGENT_HOME) —
...
def _should_exclude(rel_path: Path) -> bool:
    ...
    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``et-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/et-agent/`` — must be preserved.
        if part == "et-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside ET_AGENT_HOME.
...
def run_backup(args) -> None:
    ...
    for dirpath, dirnames, filenames in os.walk(hermes_root, followlinks=False):
        ...
        # ``et-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "et-agent" and not is_root)
        ]
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    if len(first_parts) == 1:
        ...
        if prefix in {".et-agent", "et-agent"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        # Guidance
        print()
        if not (hermes_root / "et-agent").is_dir():
            print("Note: The et-agent codebase was not included in the backup.")
...
# Critical state files to include in quick snapshots (relative to ET_AGENT_HOME).
...
# Relative path of the cron job database inside ET_AGENT_HOME. Kept in sync with
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of ET_AGENT_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<ET_AGENT_HOME>/backups/pre-update-<timestamp>.zip`` and
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of ET_AGENT_HOME under ``backups/`` before a
    ...
    ``<ET_AGENT_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
