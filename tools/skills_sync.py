...
Copies bundled skills from the repo's skills/ directory into ~/.et-agent/skills/
...
The manifest lives at ~/.et-agent/skills/.bundled_manifest.
"""
...
ET_AGENT_HOME = get_hermes_home()
SKILLS_DIR = ET_AGENT_HOME / "skills"
...
# When present in ET_AGENT_HOME, sync_skills() is a no-op so neither the
...
def _get_bundled_dir() -> Path:
    ...
    Checks ET_AGENT_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.et-agent/skills/.curator_suppressed`` directly if that import
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.et-agent/skills/mlops/axolotl
    """
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.et-agent/skills/ using the manifest.
    ...
    if (ET_AGENT_HOME / NO_BUNDLED_SKILLS_MARKER).exists():
        if not quiet:
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.et-agent/skills/.curator_suppressed) is written when the curator
...
def _rmtree_writable(path: Path) -> None:
    ...
    # ``ET_AGENT_HOME/skills/`` to prevent the catastrophic wipe of
    # ``~/.et-agent/`` (``.env``, ``MEMORY.md``, ``kanban.db``, custom
    ...
    # path join, a missing ``ET_AGENT_HOME`` default, a malicious
    ...
    # ``ET_AGENT_HOME``. Require a strict-child relationship so both escape
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    When ``enabled`` is True, writes ET_AGENT_HOME/.no-bundled-skills so the
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = ET_AGENT_HOME / NO_BUNDLED_SKILLS_MARKER
    existed = marker.exists()
    try:
        if enabled:
            ET_AGENT_HOME.mkdir(parents=True, exist_ok=True)
            marker.write_text(
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (ET_AGENT_HOME / NO_BUNDLED_SKILLS_MARKER).exists()
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.et-agent/skills/ ...")
