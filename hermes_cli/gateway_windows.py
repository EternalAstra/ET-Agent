...
_TASK_DESCRIPTION = "ET-Agent Gateway - Messaging Platform Integration"
...
def get_task_script_path() -> Path:
    ...
    (or ``<ET_AGENT_HOME>/gateway-service/<task_name>.cmd`` so per-profile
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``ET_AGENT_HOME`` whenever it
    ...
    source checkout only if ``ET_AGENT_HOME`` cannot be resolved yet.
    """
    from hermes_cli.config import get_hermes_home
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports ET_AGENT_HOME, PYTHONIOENCODING, VIRTUAL_ENV
    ...
    lines.append(f'set "ET_AGENT_HOME={hermes_home}"')
