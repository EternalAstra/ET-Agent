...
transport, discovers their tools, and registers them into the et-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.et-agent/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.et-agent/logs/mcp-stderr.log), tagged with the
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            hermes_home = os.path.expanduser(
                os.getenv(
                    "ET_AGENT_HOME", os.path.join(os.path.expanduser("~"), ".et-agent")
                )
            )
            candidates = [
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "  pip install 'et-agent[mcp]'\n"
                "or (full install):\n"
                "  pip install 'et-agent[all]'"
            )
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"file under ~/.et-agent/mcp-tokens/ and restart). Do NOT retry "
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local ET_AGENT_HOME override into ``coro``.
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local ET_AGENT_HOME override onto the MCP loop.
...
def _load_mcp_config() -> Dict[str, dict]:
    ...
    ``os.environ`` (which includes ``~/.et-agent/.env`` loaded at startup).
    """
    try:
