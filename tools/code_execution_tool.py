...
def _execute_remote(
    ...
    try:
        ...
        tz = os.getenv("ET_AGENT_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    try:
        ...
        # Ensure the et-agent root is importable in the sandbox so
        ...
        # ET_AGENT_TIMEZONE is an internal Hermes setting and must not leak
        # into child processes.
        _tz_name = os.getenv("ET_AGENT_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("ET_AGENT_TIMEZONE", None)
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.et-agent/.env')).
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (et-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    # temp-dir staging and et-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.et-agent/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
