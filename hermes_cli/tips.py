...
TIPS = [
    ...
    "ET_AGENT_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    "Checkpoints use shadow repos in ~/.et-agent/checkpoints/ — your project's .git is never touched.",
    ...
    "Cron scripts live in ~/.et-agent/scripts/ and run before the agent — perfect for data collection pipelines.",
    ...
    "The skills audit log at ~/.et-agent/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope Hermes state via ET_AGENT_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "ET_AGENT_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in ET_AGENT_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.et-agent/interrupt_debug.log with timestamps.",
    ...
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.et-agent/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.et-agent/skills/.hub/quarantine/ holds skills pending security review.",
    ...
    '/reload reloads ~/.et-agent/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.et-agent/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.et-agent/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',
    ...
    'Drop a YAML in ~/.et-agent/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.et-agent/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.et-agent/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.et-agent/skills/ so drop-in skills appear without restarting the session.',
    ...
    'ET_AGENT_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.hermes/plugins/ — trust-gated by design.',
    ...
    'ET_AGENT_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'ET_AGENT_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.et-agent/dashboard-plugins/.',
]
