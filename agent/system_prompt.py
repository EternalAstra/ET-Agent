...
upstream prefix cache warm.  See ``et-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    # Some execution modes (cron) still want ET_AGENT_HOME persona while keeping
    ...
    # Pointer to the et-agent skill + docs for user questions about Hermes itself.
    ...
    # under so it doesn't conflate ~/.et-agent/skills/ (default profile) with
    # ~/.et-agent/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            ...
            "under ~/.et-agent/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            ...
            f"and writes ~/.et-agent/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.et-agent/skills/, ~/.et-agent/plugins/, "
            f"~/.et-agent/cron/, ~/.et-agent/memories/ — those belong to a "
