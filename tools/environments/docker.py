...
def _load_hermes_env_vars() -> dict[str, str]:
    """Load ~/.et-agent/.env values without failing Docker command execution."""
    try:
...
def reap_orphan_containers(
    ...
    * ``label=et-agent=1`` (created by this codebase)
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    filters = ["--filter", "label=et-agent=1", "--filter", "status=exited"]
    if profile_filter:
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``et-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.et-agent/sandboxes/). Non-persistent
        ...
        # s6-overlay images (e.g. et-agent:latest) already use /init as PID 1
        ...
        #   * the orphan reaper (`et-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`hermes-task-id`, `hermes-profile`)
        #   * operators running `docker ps --filter label=et-agent=1`
