...
# ET_AGENT_HOME paths we've already pulled external secrets for during this
...
def reset_secret_source_cache() -> None:
    """Forget which ET_AGENT_HOME paths have already had external secrets applied.
...
def load_hermes_dotenv(
    *,
    ...
    Behavior:
    - `~/.et-agent/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(hermes_home or os.getenv("ET_AGENT_HOME", Path.home() / ".et-agent"))
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of ET_AGENT_HOME / profile). v1
