...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.et-agent/auth.json active provider)
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://et-agent.nousresearch.com",
    "X-Title": "ET-Agent",
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (ET-Agent)",
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.et-agent/auth.json for an active Nous provider.
...
def _resolve_auto(
    ...
    #    old OPENAI_BASE_URL lingers in ~/.et-agent/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "OPENAI_BASE_URL from ~/.et-agent/.env",
