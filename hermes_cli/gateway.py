...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        # the command doesn't advertise *some other* profile. ET_AGENT_HOME
...
def _sync_hermes_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``ET_AGENT_HOME``.

    Under ``sudo``, ``ET_AGENT_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_hermes_home` falls back to ``/root/.hermes`` — the wrong
    profile. The unit file pins ``ET_AGENT_HOME`` for the actual gateway
    ...
    unit_home = env.get("ET_AGENT_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("ET_AGENT_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["ET_AGENT_HOME"] = unit_home
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (ET_AGENT_HOME-scoped).
...
SERVICE_DESCRIPTION = "ET-Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current ET_AGENT_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (ET_AGENT_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary ET_AGENT_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(hermes_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when ET_AGENT_HOME is a named profile.

    For ``~/.et-agent/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        hermes_home: Optional explicit ET_AGENT_HOME path. Defaults to the current
...
def _profile_arg_for_target_user(hermes_home: str, target_home_dir: str) -> str:
    ...
    target_root = Path(target_home_dir) / ".et-agent"
    try:
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this ET_AGENT_HOME.

    Default ``~/.hermes`` returns ``hermes-gateway`` (backward compatible).
    Profile ``~/.et-agent/profiles/coder`` returns ``hermes-gateway-coder``.
    Any other ET_AGENT_HOME appends a short hash for uniqueness.
    """
...
def get_launchd_plist_path() -> Path:
    ...
    Profile ``~/.et-agent/profiles/coder`` → ``ai.hermes.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.hermes/et-agent  -> /home/alice/.hermes/et-agent
...
def _hermes_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current ET_AGENT_HOME to the equivalent under a target user's home.
    ...
    current_default = (Path.home() / ".et-agent").resolve()
    target_default = Path(target_home_dir) / ".et-agent"
...
def _stable_service_working_dir() -> str:
    ...
    ``ET_AGENT_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if ET_AGENT_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        # Anchor cwd to the target user's ET_AGENT_HOME (stable, always exists)
...
Environment="ET_AGENT_HOME={hermes_home}"
Restart=always
RestartSec=5
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir ET_AGENT_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved ET_AGENT_HOME
    ...
    exported a throwaway ``ET_AGENT_HOME=/tmp/...`` — writing it to the real
    ...
    with ``ET_AGENT_HOME=/tmp/hermes-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="ET_AGENT_HOME=..."`` lines and launchd
    ``<key>ET_AGENT_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'ET_AGENT_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>ET_AGENT_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp ET_AGENT_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: ET_AGENT_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported ET_AGENT_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only ET_AGENT_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp ET_AGENT_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../hermes_test`` path into
    # ``Environment="ET_AGENT_HOME=..."``. Writing that to the developer's
    ...
    # ET_AGENT_HOME into the unit (manual E2E homes like /tmp/hermes-e2e-NNN
...
def _gateway_run_command() -> list[str]:
    ...
    Profile-aware: honors the active ET_AGENT_HOME via `_profile_arg()` so the
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    unrelated gateway processes from other ET_AGENT_HOME roots.
    """
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print(
        ...
        "$ET_AGENT_HOME and break later non-root dashboard/gateway runs."
    )
    print(
...
def _all_platforms() -> list[dict]:
    ...
    # User-installed platform plugins under ~/.et-agent/plugins/ still require
...
def _setup_weixin():
    ...
    print_info(
        "  3. Hermes will store the returned account_id/token in ~/.et-agent/.env."
    )
    print_info(
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.et-agent/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.et-agent/.env: {', '.join(required)}")
    else:
        print_info(
...
def _dispatch_via_service_manager_if_s6(
    ...
    if profile is None:
        ...
        # ET_AGENT_HOME=<root>/profiles/<name>, "" for the default root,
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    # `docker logs` and ${ET_AGENT_HOME}/logs/gateways/<profile>/current,
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            ...
            print(
                "  nohup hermes gateway run > ~/.et-agent/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    print(
                        "  nohup hermes gateway run > ~/.et-agent/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s hermes 'hermes gateway run'         # persistent via tmux"
