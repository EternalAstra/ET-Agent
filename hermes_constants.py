...
_UNSET = object()
_ET_AGENT_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_ET_AGENT_HOME_OVERRIDE", default=_UNSET
)
...
def _get_platform_default_hermes_home() -> Path:
    ...
    if sys.platform == "win32":
        ...
        return base / "et-agent"
    return Path.home() / ".et-agent"


def get_hermes_home() -> Path:
    """Return the Hermes home directory (default: platform-native path).

    Reads ET_AGENT_HOME env var, falls back to the platform-native default.
    This is the single source of truth — all other copies should import this.

    When ``ET_AGENT_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``ET_AGENT_HOME`` explicitly (see the systemd
    template in ``hermes_cli/gateway.py`` and the kanban dispatcher in
    ``hermes_cli/kanban_db.py``).  See https://github.com/NousResearch/et-agent/issues/18594.
    """
    override = get_hermes_home_override()
    if override:
        return Path(override)

    val = os.environ.get("ET_AGENT_HOME", "").strip()
    if val:
        return Path(val)
    ...
    if not _profile_fallback_warned:
        ...
        if active and active != "default":
            ...
            msg = (
                f"[ET_AGENT_HOME fallback] ET_AGENT_HOME is unset but active "
                f"profile is {active!r}. Falling back to {fallback_home}, which "
                f"is the DEFAULT profile — not {active!r}. Any data this "
                f"process writes will land in the wrong profile. The "
                f"subprocess spawner should pass ET_AGENT_HOME explicitly "
                f"(see issue #18594)."
            )
            try:
...
def get_default_hermes_root() -> Path:
    ...
    (``~/.hermes`` on POSIX, ``%LOCALAPPDATA%\et-agent`` on native Windows).

    In Docker or custom deployments where ``ET_AGENT_HOME`` points outside
    ``~/.hermes`` (e.g. ``/opt/data``), returns ``ET_AGENT_HOME`` directly
    — that IS the root.

    In profile mode where ``ET_AGENT_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.et-agent/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_hermes_home()
    env_home = os.environ.get("ET_AGENT_HOME", "")
    if not env_home:
        return native_home
    env_path = Path(env_home)
    try:
        env_path.resolve().relative_to(native_home.resolve())
        # ET_AGENT_HOME is under ~/.hermes (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — ET_AGENT_HOME itself is the root
    return env_path
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``ET_AGENT_OPTIONAL_SKILLS``.
    """
    override = os.getenv("ET_AGENT_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``ET_AGENT_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Wheel-installed ``<sysconfig data>/skills`` (pip install path)
        3. Caller-supplied ``default`` (typically the source-checkout path)
        4. ``<ET_AGENT_HOME>/skills`` last-resort
    """
    override = os.getenv("ET_AGENT_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
...
def get_hermes_dir(new_subpath: str, old_name: str) -> Path:
    ...
    Args:
        new_subpath: Preferred path relative to ET_AGENT_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to ET_AGENT_HOME (e.g. ``"image_cache"``).
...
def display_hermes_home() -> str:
    """Return a user-friendly display string for the current ET_AGENT_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.hermes``
        profile:  ``~/.et-agent/profiles/coder``
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``ET_AGENT_HOME`` or other path
    env vars resolve to an unexpected location.

    See https://github.com/NousResearch/et-agent/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{ET_AGENT_HOME}/home`` when the profile-home directory exists."""
    hermes_home = get_hermes_home_override() or (env or {}).get("ET_AGENT_HOME") or os.getenv("ET_AGENT_HOME")
    if not hermes_home:
        return None
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    ...
    ``ET_AGENT_HOME`` scopes Hermes state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={ET_AGENT_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{ET_AGENT_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{ET_AGENT_HOME}/home`` when it exists, preserving the
...
def is_container() -> bool:
    ...
    See: NousResearch/et-agent#47111
    """
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under ET_AGENT_HOME.
...
def get_skills_dir() -> Path:
    """Return the path to the skills directory under ET_AGENT_HOME."""
    return get_hermes_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under ET_AGENT_HOME."""
