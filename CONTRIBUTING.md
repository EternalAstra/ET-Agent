# Contributing to ET-Agent

Thank you for contributing to ET-Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.et-agent/plugins/` (or via a pip entry point).
...
`$HERMES_HOME/et-agent` (usually `~/.et-agent/et-agent`). That keeps your
...
```bash
mkdir -p ~/.et-agent/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.et-agent/config.yaml
touch ~/.et-agent/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.et-agent/.env
```
...
│
├── skills/                   # Bundled skills (copied to ~/.et-agent/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (et-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.et-agent/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.et-agent/`)

| Path | Purpose |
|------|---------|
| `~/.et-agent/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.et-agent/.env` | API keys and secrets |
| `~/.et-agent/auth.json` | OAuth credentials (Nous Portal) |
| `~/.et-agent/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.et-agent/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.et-agent/state.db` | SQLite session database |
| `~/.et-agent/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.et-agent/cron/` | Scheduled job data |
| `~/.et-agent/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`hermes_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.et-agent/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `hermes setup` or update `~/.et-agent/.env` locally.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "ET-Agent" is the secondary collaborator. If the contributor's commit shows "ET-Agent" as author because they used Hermes to draft the skill, replace it with their actual name — credit the human, not the tool.
...
Create `~/.et-agent/skins/<name>.yaml`:
...
- Use [GitHub Issues](https://github.com/NousResearch/et-agent/issues)
