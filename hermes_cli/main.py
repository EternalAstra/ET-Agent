...
Usage:
    ...
    hermes uninstall           Uninstall ET-Agent
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("ET_AGENT_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".et-agent", "config.yaml")
...
def _print_fast_version_info() -> None:
    ...
    print(f"ET-Agent v{__version__} ({__release_date__})")
...
#
# Many modules cache ET_AGENT_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("ET_AGENT_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.et-agent/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set ET_AGENT_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        ...
        candidate = home / ".et-agent" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # 1.5 If ET_AGENT_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.et-agent/profiles/coder or
    # /opt/data/profiles/coder).  If ET_AGENT_HOME points to the hermes root
    # instead (e.g. systemd hardcodes ET_AGENT_HOME=/root/.hermes), we must
    # still read active_profile — the user may have switched profiles via
    # `hermes profile use` and the gateway should honour that choice.
    # See issue #22502.
    hermes_home_env = os.environ.get("ET_AGENT_HOME", "")
    ...
    # "the root ET_AGENT_HOME profile". If the reserved default child read
    ...
    # 3. If we found a profile, resolve and set ET_AGENT_HOME
    if profile_name is not None:
        try:
            ...
            return
        os.environ["ET_AGENT_HOME"] = hermes_home
...
# Load .env from ~/.et-agent/.env first, then project root as dev fallback.
...
def _ensure_tui_node() -> None:
    ...
    hermes_home = os.environ.get("ET_AGENT_HOME") or str(Path.home() / ".et-agent")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "ET_AGENT_HOME": hermes_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
...
def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.et-agent/skills/`` with the bundled skill library on first launch.
...
def cmd_chat(args):
    ...
    # user's ~/.et-agent/config.yaml and return built-in defaults. Set BEFORE
...
def cmd_whatsapp(args):
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '⚕ ET-Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
...
def select_provider_and_model(args=None):
    ...
    # a leftover OPENAI_BASE_URL in ~/.et-agent/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.et-agent/.env if the active provider is not 'custom'.
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    ...
    recover from a malformed paste without editing ``~/.et-agent/.env`` by hand.
...
def cmd_uninstall(args):
    """Uninstall ET-Agent (or just the Chat GUI with --gui)."""
...
#
# Stamp file: $ET_AGENT_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $ET_AGENT_HOME."""
    from hermes_constants import get_hermes_home
...
def _print_curator_first_run_notice() -> None:
    ...
    print(
        "  Docs:         https://et-agent.nousresearch.com/docs/user-guide/features/curator"
    )
...
def _update_via_zip(args):
    """Update ET-Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        sys.exit(1)
    zip_url = (
        f"https://github.com/NousResearch/et-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="hermes-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"et-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for et-agent itself
        ...
        # GitHub ZIPs extract to et-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"et-agent-{branch}")
...
OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/et-agent.git",
    "git@github.com:NousResearch/et-agent.git",
    "https://github.com/NousResearch/et-agent",
    "git@github.com:NousResearch/et-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/et-agent.git"
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print("  This means you may miss updates from NousResearch/et-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/et-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/et-agent.git' to add later."
            )
...
# never landed).  Lives next to the venv (not under $ET_AGENT_HOME) because the
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$ET_AGENT_HOME/bin/uv``, but on Termux the official
...
class _UpdateOutputStream:
    ...
    * Every write is also mirrored to an append-only log file
      (``~/.et-agent/logs/update.log``) that users can inspect after the
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.et-agent/logs/update.log`` and to silently absorb
...
def _ensure_fhs_path_guard() -> None:
    ...
    # /usr/local/bin/hermes, code at /usr/local/lib/et-agent).
    ...
    path_comment = (
        "# ET-Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of ET_AGENT_HOME before running the update.
    ...
    ET_AGENT_HOME directories.  The ``--backup`` flag on ``hermes update``
    ...
    # Render path using display_hermes_home so the user sees ~/.et-agent/...
    try:
...
def cmd_update(args):
    """Update ET-Agent to the latest version.
...
def _cmd_update_pip(args):
    ...
    if is_uv_tool_install():
        ...
        cmd = [pipx, "upgrade", "et-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "et-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "et-agent"]
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://et-agent.nousresearch.com/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        # though `git pull` can't touch $ET_AGENT_HOME, this is cheap
        ...
        # current — so copy it straight over ~/.et-agent/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit ET_AGENT_HOME so
        # it is not affected by sync_skills()'s module-level ET_AGENT_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's ET_AGENT_HOME env var points at the default or a named profile.
        try:
...
def cmd_dashboard(args):
    ...
    # launches the dashboard (`worker dashboard` → ET_AGENT_HOME points into
    ...
    if (
        ...
        # ET_AGENT_HOME.  We must resolve the root explicitly instead of just
        # dropping ET_AGENT_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV ET_AGENT_HOME=/opt/data`), so an unset
        # ET_AGENT_HOME falls back to $HOME/.hermes = /opt/data/.hermes — an
        ...
        try:
            from hermes_constants import get_default_hermes_root
            env["ET_AGENT_HOME"] = str(get_default_hermes_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop ET_AGENT_HOME) rather than block the reroute.
            env.pop("ET_AGENT_HOME", None)
...
def cmd_acp(args):
    """Launch ET-Agent as an ACP server."""
    try:
...
def main():
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://et-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.et-agent/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://et-agent.nousresearch.com/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.et-agent/checkpoints/",
