...
echo "HONCHO_API_KEY=***" >> ~/.et-agent/.env
```
...
| 2 | `~/.et-agent/honcho.json` | Default profile (shared host blocks) |
...
| 5 | `per-repo` strategy | Git root directory name (`et-agent`) |
...
If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-et-agent`.
