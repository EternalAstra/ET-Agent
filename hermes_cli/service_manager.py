...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's ET_AGENT_HOME blindly;
    derive the shared profile root from the current ET_AGENT_HOME and map the
    ...
    hermes_home = Path(os.environ.get("ET_AGENT_HOME", "/opt/data"))
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources ET_AGENT_HOME (and any extra env) via with-contenv —
             so e.g. ``-e ET_AGENT_HOME=/data/hermes`` is honored at run
        ...
        root ET_AGENT_HOME profile" (the implicit profile that exists at
        the top of $ET_AGENT_HOME, not under profiles/). It must be
        ...
        would instead look up ``$ET_AGENT_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``ET_AGENT_HOME``,
        ...
        # profile's ET_AGENT_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``hermes gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-ET_AGENT_HOME PID lock first;
        ...
        # Each profile is scoped to its own ET_AGENT_HOME and s6 guarantees a
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${ET_AGENT_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the ET_AGENT_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e ET_AGENT_HOME=/data/hermes``
        ...
        return (
            ...
            f': "${{ET_AGENT_HOME:=/opt/data}}"\n'
            f'log_dir="$ET_AGENT_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            ...
            f'chown hermes:hermes "$ET_AGENT_HOME/logs/gateways" 2>/dev/null || true\n'
